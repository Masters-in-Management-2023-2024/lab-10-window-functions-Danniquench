...
Original file is located at
    https://colab.research.google.com/github/Masters-in-Management-2023-2024/lab-10-window-functions-Danniquench/blob/main/lab_example.ipynb
...
dt_col = "Collision Date and Time"

if strikes.schema.get(dt_col) == pl.Utf8:
    strikes = strikes.with_columns(
        pl.col(dt_col)
          .str.to_datetime(format="%Y-%m-%dT%H:%M:%SZ", utc=True, strict=False)
          .alias(dt_col)
    )

# Running total of strikes by day through 2013
running_total_strikes = (
    strikes
    .with_columns(pl.col(dt_col).dt.date().alias("date"))
    .group_by("date")
    .agg(pl.col("Number of Strikes").sum().alias("daily_strikes"))
    .sort("date")
    .with_columns(
        pl.col("daily_strikes").cum_sum().alias("strikes_cumulative")  # <-- use cum_sum()
    )
